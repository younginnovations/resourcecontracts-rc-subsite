...
<table class="table table-responsive table-contract table-contract-list">
	...
	@if(!site()->isCountrySite())
		<th width="12%">
			<a href="{{appendInUrl($route,$url,"country",$order)}}">@lang('global.country') {!!show_arrow($order,$sortBy=='country')!!}</a>
		</th>
	@endif
	@if($showYear)
		<th>
			<a href="{{appendInUrl($route,$url,"year",$order)}}">@lang('global.year') {!!show_arrow($order, $sortBy=='year')!!}</a>
		</th>
	@endif
	<th width="13%">
	...
	<th width="25%">
		<a href="{{appendInUrl($route,$url,"contract_type",$order)}}">@lang('contract.contract_type') {!!show_arrow($order, $sortBy=='contract_type')!!}</a>
	</th>
	</thead>
	<tbody>
	@forelse($contracts->results as $contract)
		...
		<tr>
			...
			<td>
				...
				@else
					<a class="title-{{$contract->open_contracting_id}}"
					   href="{{route('contract.detail',['id'=>$contract->open_contracting_id ])}}">
						{{ $contract->name or ''}}
					</a>
				@endif
				<?php
				...
				@if($annotations->total>0)
					<div class="annotate-text" data-popover="true" data-html=true data-content="
                    @if(site()->showAnnotationLink())
					@lang('global.annotated' , ['link' => url($link)])
					@else
					@lang('global.annotated_no_link')
					@endif
							"></div>
				@endif
				...
				<div class="search-text">
					...
					@if(isset($contract->metadata ) && $contract->metadata !='')
						<p>
							<a href="{{ route('contract.view' , ['id' => $contract->open_contracting_id]) }}">{!! $contract->metadata.'...' !!}
				...
				<div class="contract-info-section">
					<div class="download-main-wrap">
						...
						<ul class="dropdown-menu">
							...
							@if(site()->canDownloadWordFile() && $contract->is_ocr_reviewed == true)
								<li>
	...
	@empty
		<tr>
			<td colspan="100%"
				class="search-not-found">@lang('search.search_not_found' , ['link' => route('contracts')])</td>
		</tr>
	@endforelse
	</tbody>
</table>