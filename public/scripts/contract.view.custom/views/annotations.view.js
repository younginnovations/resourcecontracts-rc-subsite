...
var AnnotationHeader = React.createClass({
    ...
    render: function () {
        ...
        if(count == 0){
           return ( <div className="annotation-title">{count} {lang.annotations} </div>)
        }
        return (
            <div className="annotation-title">{count} {lang.annotations}
            <span className="pull-right">
                    <a href={this.props.contractApp.getAnnotationsListAnchor()}>{lang.see_all}</a>
            </span>
            </div>
        );
    }
        ...
        if (this.props.prevAnnotation === undefined || this.props.showClusterAnyway) {
            return (
                <div className={currentAnnotationClass} id={this.state.id}>
                    ...
                    <span className="link annotation-category-en"><a href="#"
                                                                     onClick={this.handleAnnotationClick}>{this.state.categoryEn}</a></span>
                    <span className="link annotation-category-fr"
                          onClick={this.handleAnnotationClick}>{this.state.categoryFr}</span>
                    <span className="annotation-item-content">{showText}
                    ...
                    <span className="link annotation-item-page"
                          onClick={this.handleAnnotationClick}>Page {this.state.pageNo}</span>
                </div>
            );
            ...
            return (
                <span id={this.state.id} className="link annotation-item-page"
                      onClick={this.handleAnnotationClick}>, {this.state.pageNo}</span>
            );
        }
        // return (
        //     <div className={currentAnnotationClass} id={this.state.id}>
    ...
    sortByCluster: function () {
        ...
        _.map(clusters, function (cluster) {
            ...
            if (filtered.length) {
                ...
                annotationsList.push(<div id={cluster}
                                          key={cluster}>{self.getAnnotationItemsComponent(newCol, false)}</div>);
            }
        });
        return (
            <div className="annotations-list" id="id-annotations-list">
